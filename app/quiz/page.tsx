...
import {
  ...
  Trophy, Zap, Flame, Shield, Wind, Heart,
} from 'lucide-react';
...
const SUB_GOALS: Record<string, string[]> = {
...
export default function QuizPage() {
  ...
  const toggleDiet = (val: string) =>
    setData(prev => ({
      ...prev,
      dietaryRestrictions: prev.dietaryRestrictions.includes(val)
        ? prev.dietaryRestrictions.filter(x => x !== val)
        : [...prev.dietaryRestrictions, val],
    }));

  const canNext = (): boolean => {
    if (step === 1) {
      const hasSize = data.unit === 'metric'
        ? !!(data.heightCm && data.weightKg)
        : !!(data.heightFt && data.weightLbs);
      return !!(data.gender && data.age && hasSize);
    }
        ...
        <div className="w-full max-w-lg fade-up" key={step}>
            ...
            <div>
                ...
                {(['metric', 'imperial'] as const).map(u => (
                  <button key={u} onClick={() => set('unit', u)}
                    className={`flex-1 py-2.5 rounded-lg text-sm font-medium border transition-all ${
                      data.unit === u ? 'bg-orange-500 border-orange-500 text-white' : 'border-white/10 text-gray-500 hover:border-white/25'
                    }`}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input type="number" min="13" max="80" value={data.age}
                ...
                <div>
                  ...
                  {data.unit === 'metric' ? (
                    <input type="number" value={data.heightCm}
                      ...
                      placeholder="Height in cm  (e.g. 178)" className={inp} />
                  ) : (
                    <div className="flex gap-3">
                      <input type="number" value={data.heightFt} min="3" max="8"
                        onChange={e => set('heightFt', e.target.value)}
                        placeholder="ft" className={inp} />
                      <input type="number" value={data.heightIn} min="0" max="11"
                        onChange={e => set('heightIn', e.target.value)}
                        placeholder="in" className={inp} />
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {data.unit === 'metric' ? (
                    <input type="number" value={data.weightKg}
                      ...
                      placeholder="Weight in kg  (e.g. 75)" className={inp} />
                  ) : (
                    <input type="number" value={data.weightLbs}
                      onChange={e => set('weightLbs', e.target.value)}
                      placeholder="Weight in lbs  (e.g. 165)" className={inp} />
                  )}
                </div>
              </div>
            </div>
          ...
          {step === 2 && (
            <div>
              ...
              <div className="space-y-2.5">
                {GOAL_OPTIONS.map(({ id, label, desc, icon: Icon, tc, bg }) => (
                  <button key={id}
                    onClick={() => { set('primaryGoal', id); set('secondaryGoal', ''); }}
                    className={`w-full flex items-center gap-4 p-4 rounded-xl border text-left transition-all card-hover ${
                      data.primaryGoal === id
                        ? 'border-orange-500 bg-orange-500/5'
                        : 'border-white/10 hover:border-white/20'
                    }`}>
                    <div className={`w-10 h-10 rounded-lg flex-shrink-0 flex items-center justify-center ${
                      data.primaryGoal === id ? bg : 'bg-white/5'
                    }`}>
          ...
          {step === 3 && (
            <div>
              ...
              <div>
                ...
                <textarea
                  value={(SUB_GOALS[data.primaryGoal] || []).includes(data.secondaryGoal) ? '' : data.secondaryGoal}
                  onChange={e => set('secondaryGoal', e.target.value)}
                  placeholder="e.g. I want to be able to throw a knockout punch…"
          ...
          {step === 4 && (
            <div>
              ...
              <p className="text-gray-500 mb-8 text-sm">Determines exercise selection and how fast we progress you.</p>
              <div className="space-y-3">
                {[
                  ...
                  <button key={id} onClick={() => set('experience', id)}
                    className={`w-full flex items-center justify-between p-5 rounded-xl border transition-all card-hover ${
                      data.experience === id ? 'bg-orange-500/10 border-orange-500' : 'border-white/10 hover:border-white/20'
                    }`}>
                    <div className="text-left">
                      <p className={`font-bold text-lg ${data.experience === id ? 'text-orange-300' : 'text-white'}`}>{label}</p>
                      <p className="text-gray-500 text-sm mt-0.5">{desc}</p>
                    </div>
                    <span className={`text-xs px-2.5 py-1 rounded-full border flex-shrink-0 ml-4 ${
                      data.experience === id ? 'border-orange-500/40 text-orange-400' : 'border-white/10 text-gray-600'
                    }`}>{badge}</span>
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <input type="number" value={data.targetWeight}
                    onChange={e => set('targetWeight', e.target.value)}
                    placeholder={data.unit === 'metric' ? 'e.g. 80' : 'e.g. 180'}
                    className={inp} />
                </div>
                ...
                <div>
                  ...
                  <textarea value={data.extraNotes}
                    onChange={e => set('extraNotes', e.target.value)}
                    placeholder="e.g. Bad lower back, only have dumbbells at home…"
                    rows={3} className={`${inp} resize-none`} />
                </div>
              </div>
            </div>
