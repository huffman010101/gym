...
type Tab = 'ai' | 'smarter' | 'morning' | 'day' | 'career' | 'sleep';
...
export default function Uni() {
  ...
  const [tab, setTab] = useState<Tab>(() => {
    ...
    return (['ai', 'smarter', 'morning', 'day', 'career', 'sleep'] as const).includes(t as Tab) ? (t as Tab) : 'ai';
  });
    ...
    <main className="min-h-screen bg-[#0a0a0a] bg-gradient-to-b from-sky-950/40 via-[#0a0a0a] to-[#0a0a0a] text-white pb-24">
      <div className="max-w-2xl mx-auto px-5 pt-6">
        ...
        {tab === 'morning' && (
          <div className="fade-up stagger space-y-4">
            <div className="card-premium p-5">
              <h3 className="font-bold mb-2 flex items-center gap-2"><Sun size={16} className="text-sky-400" /> The Morning Routine</h3>
            ...
            <div className="bg-gradient-to-br from-sky-500/15 to-[#111] border border-sky-500/30 rounded-2xl p-5">
              <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-sky-300/70 mb-3">The 6 non-negotiables</p>
              <div className="space-y-3">
                {[
                  ...
                  <div key={t} className="flex gap-3">
                    <span className="text-sky-400/70 font-black text-xs mt-0.5 w-4 flex-shrink-0">{i + 1}</span>
            ...
            <Fold title="Why each piece works" tag="The reasoning, so you can adapt it" items={[
              ...
              ['Cold exposure', 'A cold finish to your shower produces a sustained rise in noradrenaline and dopamine — sharper focus and better mood for hours, not minutes. It is also a daily rep of doing something you do not want to do (see Mind → Focus & Discipline).'],
            ...
            <Block title="Adapting it to your day" items={[
              ...
              ['Pair it with the night before', 'Tomorrow\'s 3 priorities written down (Home → Plan tomorrow), clothes out, phone out of the room. A good morning is mostly assembled the night before — see the Sleep Lab tab.'],
            ]} />
          </div>
        )}
