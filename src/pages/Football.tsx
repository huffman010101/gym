      ...
      <div className="max-w-2xl mx-auto px-5 pt-6">
        ...
        {tab === 'speed' && (
          <div className="fade-up stagger space-y-4">
            ...
            <Fold title="Acceleration — the physics of the first five metres" tag="Where nearly every football sprint is won" items={[
              ['You accelerate by pushing BACKWARDS, not by moving your legs faster', 'Acceleration is horizontal force into the ground. The reason elite accelerators pull away in 5m is that they orient more of their force behind them rather than straight down. Everything below is a way of doing that.'],
              ['The shin angle rule', 'In the first 3-4 steps your shin should be roughly parallel to the ground at contact, with the whole body in one line from ankle to head at about 45°. If your shin is vertical, you are pushing down and going nowhere. This is the single most correctable fault in amateur acceleration.'],
              ['Push the ground away for longer', 'Early steps have long ground-contact times on purpose — you are pushing, not bouncing. Trying to move your feet fast out of the blocks is the classic error; it makes the steps short and light and produces no force.'],
              ['Rise gradually over 10-15m, never pop up', 'Each step you come up a few degrees. Standing up in step two kills the whole thing. Drill it by starting from a falling position (lean until you have to catch yourself, then run).'],
              ['Arms drive backwards and hard', 'Elbows punch back, not across. The arm action is what allows the opposite leg to apply force — sloppy arms genuinely cost you metres.'],
              ['Drills that build it', 'Wall drives (3×5 each side, holding the 45° line), falling starts (6×15m), sled pushes or heavy sled marches (4×20m), and hill sprints (6×15m) — a hill forces the correct lean automatically, which is why it is the best self-coaching tool you have.'],
              ['The gym side of acceleration', 'Horizontal force comes mostly from the glutes and hamstrings: hip thrusts, RDLs, Bulgarian split squats, and sled work. Trap bar jumps and broad jumps convert that strength to speed. Vertical-only training (squats and box jumps alone) builds the wrong direction for football.'],
            ]} />
            <Fold title="Change of direction — deceleration is the actual skill" tag="Nobody trains the braking half, and it is the half that beats defenders" items={[
              ['Agility = brakes, not turning', 'Your ability to change direction is limited almost entirely by how fast you can STOP. Eccentric strength — absorbing force — is the bottleneck, and it is the least-trained quality in amateur football.'],
              ['The penultimate step does the work', 'The second-to-last step before a cut is where you drop your centre of mass and load the brake; the last step redirects. Watch any elite winger frame by frame and you will see that low, wide penultimate step. Coaching yourself to "get low the step BEFORE you cut" fixes most cutting problems.'],
              ['Cut angles behave differently', 'Under ~45°: you barely need to slow down, so it is a speed-and-lean problem — this is the sidestep that beats a defender at pace. 45-90°: real braking required, plant hard on the outside foot with a wide base. 180° turns: pure deceleration then re-acceleration, and it is essentially a strength quality.'],
              ['Do not plant on a straight, locked leg', 'The knee should bend to absorb. A stiff, straight plant leg with the knee falling inwards is the classic ACL mechanism. Knee tracks over the foot, hips back, chest up.'],
              ['Trainable drills', '5-0-5 test and drill (sprint 15m, turn 180°, sprint 5m back) for pure deceleration; L-drill and T-drill for mixed angles; and cut-and-go patterns at 45° and 90° off both feet — almost everyone is markedly worse off their weak side.'],
              ['Reactive agility is a different skill', 'Cone drills are pre-planned; football is not. Once the pattern is clean, make it reactive: a partner points, calls a colour, or drops a ball and you react. Reaction time and decision speed are what actually separate players in games, and pre-planned drills never touch them.'],
              ['The gym side of braking', 'Eccentric-focused work: slow 4-second lowers on split squats, Nordics, drop-and-stick landings (jump off a low box, land and freeze for 2 seconds), and lateral bounds with a stuck landing. If you cannot land and hold it silently, you cannot decelerate at speed.'],
            ]} />
            <Fold title="Ankles — stiffness, strength and why yours matter here" tag="Every cut, sprint step and landing goes through them" items={[
              ['Two different qualities, both needed', 'STRENGTH stops the ankle rolling when you land awkwardly or get caught. STIFFNESS is the springiness that returns energy on every sprint step — a stiff ankle that does not collapse is free speed. They are trained differently and most people train neither.'],
              ['If your ankles are currently weak, start there', 'Do not add cutting and plyometrics on top of ankles that are not ready — that is how a niggle becomes a sprain. The full ramp-up progression (tibialis and peroneal work, balance progressions, the dorsiflexion test, an 8-week build) is in Programs → Legs. Do that first; this tab assumes ankles that already tolerate load.'],
              ['Calf work in both positions', 'Straight-leg calf raises (gastrocnemius, the sprinting muscle) 3×12 AND bent-knee seated raises (soleus, which takes the largest load in running) 3×15. Skipping the seated version is extremely common and leaves the bigger of the two under-trained.'],
              ['Balance under perturbation, not just balance', 'Standing on one leg with your eyes closed is a start. Progress it to single-leg holds while a partner lightly pushes you, or while you head a ball back — the ankle has to react to something unexpected, which is what actually happens on a pitch.'],
              ['Pogos and low hops build the stiffness', 'Straight-leg bouncing on the balls of the feet, minimal ground contact, 3×15. Then single-leg. This is what converts ankle strength into sprint speed, and it doubles as the best warm-up for the nervous system before speed work.'],
              ['Boots and surfaces are part of it', 'Blades and long studs on a hard, dry pitch increase the twisting load on the ankle and knee. Match studs to the surface, and do not do your first session back in brand new boots.'],
              ['A history of sprains needs deliberate work', 'A previously sprained ankle stays measurably worse at proprioception for a long time unless it is specifically retrained — this is why a first sprain so reliably predicts a second. Balance and hop work close that gap; time alone does not.'],
            ]} />
